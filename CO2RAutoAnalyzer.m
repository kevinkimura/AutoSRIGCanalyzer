...
clear; 
clc; close all;
electrode_name = ['CuFoil168'];
exp_parameters = ['-1650-200mVps50ms'];
GCstartdelay = 8; %min
GCrunrest = 26; %18 min run time and 10 min rest time
Leftend = 1;
Rightend = 8;
Area = 0.2; %probably need to change
Prodnum = 6; %H2, CO, CH4, C2H4, formic, EtOH
...
products = ["H_2","CO","CH_4","C_2H_4","HCOOH","EtOH"];
product_colors = [1 0 0; 1 0.6 0; 0 0.498 0; 0.4 0.6 1; 0.4 0.2 1; 0.64 0.08 0.18];
n = [2 2 8 12 2 12];
flowrate = 1.4; %sccm
CatholyteVol = 15; %mL
...
for i = Leftend:Rightend
    ...
    COpeak1 = 4.3; %first CO peak location
...
for i = 1:len
    ...
    if ECdata.data(i,2) < -0.5 && ECdata.data(i,2) > -1.7 %% taking current data that is negative but not huge negative charging spikes. this value will change based on case
