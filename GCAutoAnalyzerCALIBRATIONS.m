...
run_num = 2; %choose the desired run #
if run_num < 10
...
%import the .ASC file
GCFID = importdata(['C:\Users\Kevin\Documents\Graduate_Work_MAIN\Electrochemistry\Data\Cufoil Data\Constant and Pulsed Long tests\190928_CuFoil184 -1650mV Const\GC Data\190928 KK CuFoil184 chelexed 100mM KHCO3 1650mv const LONG FID',runstr,'.ASC']); %
GCTCD = importdata(['C:\Users\Kevin\Documents\Graduate_Work_MAIN\Electrochemistry\Data\Cufoil Data\Constant and Pulsed Long tests\190928_CuFoil184 -1650mV Const\GC Data\190928 KK CuFoil184 chelexed 100mM KHCO3 1650mv const LONG TCD',runstr,'.ASC']); %

%GC Aug 2019 Calibration
...
% H2peak1Area = H2peak1AreaFinder(H2peak1, time, GCTCD.data);
% CH4peak1Area = CH4peak1AreaFinder(CH4peak1, time, GCFID.data);
% COpeak1Area = COpeak1AreaFinder(COpeak1, time, GCFID.data);
% CH4peak2Area = CH4peak2AreaFinder(CH4peak2, time, GCFID.data, 55, 25, 20, 55);
% C2H4peak1Area = C2H4peak1AreaFinder(C2H4peak1, time, GCFID.data, 200, 80, 60, 120);
...
% figure
% plot(time,GCTCD.data(:,1))
% hold all
% [val,idx]=min(abs(time-H2peak1)); %finding the index for Peak position
% tpseudo = [time((idx-30):(idx-18)),time((idx+30):(idx+50))];
% datapseudo = [GCTCD.data((idx-30):(idx-18)),GCTCD.data((idx+30):(idx+50))];
% timeH2 = time((idx-30):(idx+50));
% P = polyfit(tpseudo,datapseudo,4);
% 
% yfit = P(1)*timeH2.^4+P(2)*timeH2.^3+P(3)*timeH2.^2+P(4)*timeH2+P(5);
% plot(time(idx-30),GCTCD.data(idx-30,1),'o')
% plot(timeH2,yfit,'r-.');
% shiftup = min(GCTCD.data((idx-30):(idx+50)));
% figure
% hold all
% area(time((idx-30):(idx+50)), abs(shiftup)+GCTCD.data((idx-30):(idx+50)));
% area(timeH2, abs(shiftup)+yfit); %ares of baseline